...
/* =====================================================
   LAUNCH BROWSER (RENDER / LOCAL SAFE)
===================================================== */
...
${logoBase64
  ? `<img src="data:image/png;base64,${logoBase64}" style="height:20px;" /><br/>`
  : ""
}
...
/* ===============================
   AMOUNT TO WORDS (UNCHANGED)
================================ */
function amountToWords(amount, currency) {
  const number = Math.floor(Number(amount));
  return currency === "INR"
    ? `Rupees ${numberToIndianWords(number)} Only`
    : `USD ${numberToEnglishWords(number)} Only`;
}
